...
def initialize_q_table(state_space: int, action_space: int) -> NDArray:
    Qtable = np.zeros(shape=(state_space, action_space))
    return Qtable


def greedy_policy(Qtable: NDArray, state: int) -> int:
    # Exploitation: take the action with the highest state, action value
    action = Qtable[state].argmax()
    return action


def epsilon_greedy_policy(Qtable: NDArray, state: int, epsilon: float, env):
...
def main():
    ...
    n_eval_episodes = 20  # Total number of test episodes
    ...
    mean_reward, std_reward = evaluate_agent(
        test_env, max_steps, n_eval_episodes, Qtable_frozenlake, eval_seed
    )
